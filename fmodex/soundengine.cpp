...
DWORD WINAPI WatchThread(LPVOID lpParam)
{
	...
	while (1)
	{
		...
		char *list[] = { "Half-Life", "Reprocessing: Playtest", "Reprocessing", "Xash3D Launcher","Xash3D Launcher (32 bit)", NULL };
...
void CSoundEngine::Init()
{
	...
	if (m_iVersion != FMOD_VERSION)
		...
		gEngfuncs.Con_Printf("FMOD Version '%08x' is initialized.\n", FMOD_VERSION);
...
extern "C" void PM_PlaySample(const char *szFile, float fVolume, int iPitch, float *origin)
{
	...
	//gEngfuncs.Con_Printf("PM_PlaySample = %s\n", szFile);
...
{
	//gEngfuncs.Con_Printf("CL_SoundEngineReset\n");
...
extern "C" __declspec(dllexport) void CL_SoundPrecache(void *pPath)
{
	...
	//gEngfuncs.Con_Printf("CL_SoundPrecache = %s\n", pPath);
	strcpy(szPath, "sound/");
...
{
	iFlags |= SND_AMBIENT;
	//gEngfuncs.Con_Printf("CL_EmitAmbient = %s\n", pPath);
...
====================
*/
int CSoundEngine::MsgFunc_Fsound(const char *pszName, int iSize, void *pBuf)
{
	BEGIN_READ(pBuf, iSize);
	int entindex = READ_SHORT();
	int channel = READ_BYTE();
	char *pszFile = READ_STRING();
	float vol = READ_FLOAT();
	float attn = READ_FLOAT();
	int flags = READ_BYTE();
	int pitch = READ_BYTE();
	...
	gSoundEngine.PlaySound((char *)pszFile, ent->origin, flags, channel, vol, pitch, attn, NULL, entindex);
...
/*
====================
MsgFunc_PrecacheSound

====================
*/
int CSoundEngine::MsgFunc_PrecacheSound(const char *pszName, int iSize, void *pBuf)
{
	BEGIN_READ(pBuf, iSize);
	char *pszFile = READ_STRING();

	gSoundEngine.PrecacheSound(pszFile);
